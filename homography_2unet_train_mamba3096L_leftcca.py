...
from homography_model_unet_mamba3096L import HomographyModel
...
def test(args):
    ...
    with torch.no_grad():
        ...
        for i, batch_value in enumerate(test_loader):
            ...
            h_losses_array.append(h_loss.item())
            ##################################psnr, ssim比较#############################
            warp_mesh_np = ((pred_I2[0] + 1.6) * 127.5).cpu().detach().numpy().transpose(1, 2, 0)
            # warp_mesh_mask_np = mask_I2[0].cpu().detach().numpy().transpose(1, 2, 0)
            inpu1_np = ((I2_aug_batch[0] + 1.6) * 127.5).cpu().detach().numpy().transpose(1, 2, 0)
...
def main():
    ...
    MODEL_DIR = MAIN_LOG_PATH + "models/synthetic_models_unet_mamba3096L"
